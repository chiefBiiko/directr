...
def sd_4src(dirs=[os.getcwd()], mode=['r', 'py', 'js'], log=False, walk=False, display=False, delay=10.):
    ...
    Args:
        ...
        'r', 'py', 'js', 'c', 'java', 'markup', 'markdown', 'css', 'txt', 'misc'
        ...
        for d in dirs:
            for rt, drs, fls in scandir.walk(d):
                if '.git' in drs:  # don't go into any .git directories
                    drs.remove('.git')
                d_src = [str(f) for f in fls if f.lower().endswith(md)]
                g_src.extend([str(os.path.join(rt, fl)) for fl in d_src])
                print('#'*79 + '\nDir: {}\nSrc: {}\n'.format(rt, d_src))
                if log:
                    with open(os.path.join(os.getcwd(), 'sd_4src_log.txt'), 'a') as f:
                        f.write('#'*79 + '\nDir: {}\nSrc: {}\n'.format(rt, d_src))
